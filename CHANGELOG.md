...
- **Harness Engineering section in README** — documents the 7-layer agent infrastructure (Model & Runtime, Memory, Tooling, Orchestration, Observability, Knowledge, Goal Definition) and how lucy-ai implements each layer.
- **Auto-include config fragment** — `install.sh` now automatically injects the `$include` directive for `config/agent-fragment.json5` into `~/.openclaw/openclaw.json` during post-install. Creates `openclaw.json` if it doesn't exist, backs up existing config before modification, and skips if already present (idempotent).
