...
description: >
  Universal SQL code review guidance for security, maintainability, access control, data protection, SQL injection prevention, schema quality, and anti-patterns. Trigger: when reviewing SQL, migrations, stored procedures, or database-access queries.
license: Apache-2.0
metadata:
...
## OpenClaw SQL Safety Rules

- Prefer review/analysis over executing SQL.
- Never run destructive SQL (`DROP`, `TRUNCATE`, mass `DELETE`/`UPDATE`, schema changes) without explicit user approval and backup/rollback context.
- Do not run `EXPLAIN ANALYZE` or production diagnostics that execute expensive queries unless the user confirms environment safety.
- Avoid exposing PII/secrets from query results; inspect schemas/plans instead of raw data when possible.
- Database-specific guidance must be checked against the actual database engine/version and repo standards.
