...
description: >
  Ignite UI for Angular data grids: flat, tree, hierarchical, pivot, grid lite, columns, editing, selection, grouping, export, remote data, paging, state, sizing. Trigger: when implementing or reviewing Ignite UI Angular tables/grids.
license: MIT
metadata:
...
Call `read` (or equivalent) on **all** reference files identified in Step 2 **preferably in a small parallel batch when independent** — do NOT read them one at a time sequentially. You must do this even if you believe you already know the answer. Do not skip, skim, or partially read a reference file.
...
- Do not persist sensitive data in `localStorage` or client-side grid state. Store only non-sensitive UI preferences.
- UI delete/edit examples are examples only. Destructive backend mutations require confirmation flows, authorization, auditability, and explicit user approval when run outside local development.
