...
description: >
  Implement Angular views from design images using Ignite UI Angular components and theming. Trigger: when Camilo provides a screenshot/mockup/wireframe and asks to build it in an Ignite UI Angular project.
license: MIT
metadata:
...
3. **Discover components** - If Ignite UI MCP tools are available, call `list_components` with targeted filters; otherwise use the bundled component-mapping references
4. **Look up component docs** - Call `get_doc` for every chosen component family before coding
5. **Generate/propose theme** - If Ignite UI theming MCP tools are available, use them for palette/elevation/typography/theme generation. If not, derive the theme manually from the bundled references. Prefer design tokens or scoped semantic CSS variables over raw literals; do not install/configure tooling without approval.
...
If MCP tools are available, call `list_components` with `framework: "angular"` and relevant filters to find components matching each UI pattern. Otherwise, use `references/component-mapping.md`. Common filters:
...
For every chosen component category, call `get_doc` with the doc name from `list_components` results (e.g., `name: "card"`, `framework: "angular"`). Use the doc `name` field from the MCP results, not the result title shown in the list. This is preferred before coding when MCP tools are available; otherwise rely on the bundled references and note uncertainty.
...
1. **Discover (MCP first when available)** - call `get_component_design_tokens(component)` before looking at the image for that component when tooling exists; otherwise consult theming references. Read the full token list with names, types, and descriptions. Identify which tokens correspond to visible surfaces, text, borders, icons, and interaction states.
...
3. **Generate/propose** - when tooling exists, call `create_component_theme(component, platform, licensed, tokens)` passing only the tokens whose resolved value differs from the global theme; otherwise write the smallest equivalent scoped theme manually. This produces scoped SCSS with the minimal override set.
...
3. For each Ignite UI component: discover available tokens with MCP when available, otherwise consult references; map image design tokens; resolve values to design tokens or semantic CSS variables; create the smallest scoped theme override
4. Use `get_color` after palette generation when MCP is available and a palette token can represent the final color intent
...
## Step 6: Propose DV Packages (Approval Required)

Core UI components ship with `igniteui-angular` in Open Source projects and may use `@infragistics/igniteui-angular` in licensed setups. Charts, maps, gauges, and sparklines require additional DV packages. These packages are version-sensitive: determine the installed Ignite UI version, resolve the compatible published DV package version, and propose only the package set required by the selected components; install only after explicit approval. See [references/component-mapping.md](references/component-mapping.md) for package names and import patterns.
