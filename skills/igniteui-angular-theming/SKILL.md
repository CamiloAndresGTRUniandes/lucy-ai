...
description: >
  Ignite UI for Angular theming: palettes, Sass themes, typography, elevations, component tokens, dark/light themes, spacing, sizing, and roundness. Trigger: when styling or theming Ignite UI Angular components.
license: MIT
metadata:
...
> ## Optional Ignite UI Theming MCP Server
...
Each component has its own set of design tokens (themeable CSS custom properties). Before theming a component, you must know which tokens exist. Use the **MCP tool** `get_component_design_tokens` to discover them when available; otherwise consult bundled references and vendor docs.
...
1. If MCP is available, call `get_component_design_tokens` for the parent (e.g., `date-picker`)
...
3. Prefer `get_component_design_tokens` before `create_component_theme` when MCP is available; otherwise verify token names from bundled/vendor docs
